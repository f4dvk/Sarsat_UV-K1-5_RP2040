...
|-----|------|---------|
| `[lvl]` | ~toutes les 3 s au repos | `peak` / `rms` de la fenêtre audio, min..max ADC brut, DC. **Règle le gain AF (écran SARSAT → touche `5`, HAUT/BAS), volume radio au max :** le souffle vise `rms ~2000-3500`, une balise lit plus bas et décode. |
...
| `[meter]` | tant que le mètre est actif (`m`) | par fenêtre : `rms/peak/dc/clip/adc` + une barre, pour contre-vérifier le réglage de gain AF |
...
- **`m`** — bascule le MÈTRE de niveau. Contre-vérification du réglage fait
  depuis la radio (volume au max, écran SARSAT → `5` → HAUT/BAS) : vise le
  **souffle** à `rms ~2000-3500`, `clip 0%`, `dc` proche du milieu d'échelle.
  Une salve de balise lit alors plus bas et décode.
