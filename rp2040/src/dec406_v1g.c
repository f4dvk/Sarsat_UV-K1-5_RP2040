...
/* On the RP2040 the decoded fields are read straight from BeaconInfo1G; the
 * upstream sub-decoder chatter (baudot call signs, "National use data: ...",
 * the full text report) would only pollute the debug console. Silence the
 * bare printf/fprintf here (snprintf, used to build vessel_id, is untouched).
 * Not defined for the host test build, which diffs the text report. */
#if defined(SARSAT_QUIET_DECODER)
...
/* ProtocolType and BeaconInfo1G are declared in dec406_v1g.h so that
 * sarsat_decoder.c and the host test harness can consume decoded fields
 * directly (Sarsat_UV-K1-5_RP2040 port). */
...
/* exposed (non-static) for sarsat_decoder.c / host tests */
...
static void decode_standard_location(const char *s, BeaconInfo1G *info, int frame_length) {
    ...
    if (frame_length == LONG_FRAME_BITS) {
        ...
        if (fixed_bits != 0b1101) {
            printf("Warning: Invalid fixed bits in PDF-2: 0x%X\n", fixed_bits);
        }
...
void decode_1g(const uint8_t *bits, int length) {
     ...
     * Output: fgb_bits.csv
     * Host-only: needs a filesystem + getenv (dropped on the RP2040). */
#if defined(SARSAT_HOST_DIAG)
    if (getenv("FGB_DIAG")) {
