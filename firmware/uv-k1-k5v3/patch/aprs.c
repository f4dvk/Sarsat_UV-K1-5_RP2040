...
void APRS_TimeSlice(void)
{
    ...
    /* ⚠️ FIX (2026-09-12, on user report): CMD_APRS_CONFIG (call/ssid/path/
     * symbol/digi level/KISS flag) used to be pushed to the RP2040 exactly
     * ONCE per boot -- from APRS_Init(), itself only run lazily the first
     * time anything touches the APRS subsystem, guarded by `s_inited` so it
     * never fires again on its own -- plus on an explicit menu-edit save.
     * The RP2040 keeps no config state of its own across ITS OWN reboots
     * either (see APRS_PushConfig()'s comment), so if that one push is
     * ever missed (e.g. sent before the RP2040's own UART receiver is even
     * up yet, a real possibility since the two boot independently) or
     * corrupted on the wire, nothing ever resends it -- the radio's menu
     * keeps showing "KISS: ON" (that only reads gAprsCfg, this station's
     * own local copy) while the RP2040 is silently still sitting on
     * whatever it defaulted to at its own boot (KISS off), with no visible
     * error either side. Symptom reported: KISS shows ON after a power
     * cycle but doesn't actually pass traffic until the user re-enters the
     * menu and toggles it off then on -- which works only because an edit
     * always re-pushes, not because anything about the toggle itself
     * matters. Fixed at the root instead of chasing that one flag: re-push
     * the whole config periodically (~3 s, arbitrary -- fast enough that a
     * missed boot-time push self-heals almost immediately, cheap enough
     * for a single 16-byte UART write while nothing has changed) instead
     * of relying on a single one-shot event with no retry. Kept in sync
     * with V1's identical fix. */
    {
