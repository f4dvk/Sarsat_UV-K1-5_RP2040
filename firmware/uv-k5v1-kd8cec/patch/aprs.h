...
/* The tracker always transmits on this fixed MR channel (170, 1-based / 169
 * 0-based), never on whatever VFO A/B happen to be tuned to -- same channel
 * number and convention as KD8CEC's own C-Board project (this firmware's MR
 * range goes up to MR_CHANNEL_LAST = 199 / "CH200", channel 170 is just the
 * one reserved here, not literally the last slot). The UI
 * labels it "APRS" instead of "CH170"/"MR 170" (APRS_Init() forces the stored
 * channel name). APRS_Beacon() reloads this channel's frequency / power /
 * modulation / bandwidth / offset / CTCSS from EEPROM for each burst, so the
 * user can freely retune VFO A/B (or sit on the SARSAT screen) while the
 * beacon keeps going out on its own channel; edit channel 170 like any other
 * memory channel (frequency, power, FM/narrow...) to change where it beacons. */
#define APRS_TX_CHANNEL   169u

/* exactly 56 bytes (7 x EEPROM 8-byte pages) */
typedef struct {
    ...
    uint8_t  opts;         /* bit0     : on the APRS band, light the backlight
                            ...
                            *            and just relays frames. Pushed via
                            *            APRS_PushConfig() flags byte.         */
    char     comment[15];
...
void APRS_ResyncAfGainKnob(void);     /* call before APRS_ApplyAfGain() at screen
                                       * entry, so "auto" tracks the knob as of
                                       * now instead of a stale first capture */
